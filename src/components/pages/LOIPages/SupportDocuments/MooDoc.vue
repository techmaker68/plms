<template>
  <DefineTemplate>
    <MooContent :application="application" :applicants="applicants" />
  </DefineTemplate>
  ...
  <div class="document-align">
    ...
    <button
      class="d-flex btn btn-outline-primary mb-4 ms-5 d-block btn-icon"
      @click="generatePDF"
    >
...
import MooContent from "../PdfContent/MooContent.vue";
...
export default defineComponent({
  ...
  computed: {
    ...
    downloadedFileName() {
      let fileName = `${
        this.application?.moo_ref ? this.application?.moo_ref + "-" : ""
      }MOO`;
      if (this.batchNumber) {
