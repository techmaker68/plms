...
const joinedEmails = computed(() => {
  return state.form.loi_admin_emails.join(",");
});

watchEffect(() => {
  const { loi_admin_emails } = settings.value;
  state.form = {
    loi_admin_emails: loi_admin_emails ? loi_admin_emails.split(",") : [],
  };
});
...
async function submit() {
  try {
    await saveSettings({
      loi_admin_emails: joinedEmails.value,
    });
    emit("submit");
  } catch (error) {
    ...
    <div class="no-borders-form">
      <div class="vertical-form-field">
        ...
        <Chips
          ...
          class="flex-grow-1"
          separator=","
          v-model="v$.form.loi_admin_emails.$model"
          :class="{
            'p-invalid':
              v$.form.loi_admin_emails.$invalid &&
              v$.form.loi_admin_emails.$dirty,
          }"
